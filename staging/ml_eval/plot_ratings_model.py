...
if __name__ == '__main__':
    ...
    data, labels = prepare_yelp_ratings_dataset_sklearn(ratings_path)

    X_train, y_train, X_test, y_test = create_train_test_set(data, labels, test_size=0.1)

    y_pred, _ = get_logistic_regression_rating_prediction(X_test, preprocess=False)
    y_pred += 1
    plot(y_test, y_pred, 'Logistic Regression')

    y_pred, _ = get_decision_tree_rating_prediction(X_test, preprocess=False)
    y_pred += 1
    plot(y_test, y_pred, 'Decision Tree')

    y_pred, _ = get_random_forest_rating_prediction(X_test, preprocess=False)
    y_pred += 1
