...
def read_datasets(args):
    if not os.path.exists('../query_datasets/q1/crimes.p'):
        crimes_init = pd.read_csv('../query_datasets/q1/' + args[1])
        ...
        pickle.dump(crimes, open('../query_datasets/q1/crimes.p', 'w'), protocol=pickle.HIGHEST_PROTOCOL)
    else:
        print('Crimes pickle dump found!')
        start = time.time()
        crimes = pickle.load(open('../query_datasets/q1/crimes.p', 'r'))
        end = time.time()
        print('Elapsed time(crimes): ' + str(end - start))

    if not os.path.exists('../query_datasets/q1/bl.p'):
        business_licenses_init = pd.read_csv('../query_datasets/q1/' + args[2])
        ...
        pickle.dump(business_licenses, open('/query_datasets/bl.p', 'w'), protocol=pickle.HIGHEST_PROTOCOL)
    else:
        print('Business license pickle dump found!')
        start = time.time()
        business_licenses = pickle.load(open('../query_datasets/q1/bl.p', 'r'))
        end = time.time()
...
def clean_datasets(crimes, bl):
    ...
    if not os.path.exists('../query_datasets/q1/combo.csv'):
        crimes_bl_combo = []
        with open('../query_datasets/q1/combo.csv', 'wb') as csvfile:
        ...
        start = time.time()
        crimes_bl_combo = []
        with open('../query_datasets/q1/combo.csv', 'rb') as csvfile:
    ...
    aggregate_new.to_csv('../results/q1-result.csv', sep=',')
